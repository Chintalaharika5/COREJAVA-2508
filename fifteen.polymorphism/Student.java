...
public class Student extends AbstractPerson implements Personable, Payables {
    ...
    public void calculatePayment() {
        ...
        // Adding new functionality --> process payment
        ...
        if (option == 1) {
            ...
            pp.makePayment(finalFee, bankName.getBankName());
        } else if (option == 2) {
            CreditCardPayment cardNumber = new CreditCardPayment();
            cardNumber.setCardNumber(cardNumber.getCardNumber());
            System.out.println("Paying Fee: "+finalFee);
            pp.makePayment(finalFee, cardNumber.getCardNumber());
        } else if (option == 3) {
            UPIPayment upiId = new UPIPayment();
            upiId.setUpiId(upiId.getUpiId());
            System.out.println("Paying Fee: "+finalFee);
            upiId.setUpiPaymentType(upiId.getUpiPaymentType());
            pp.makePayment(finalFee, upiId.getUpiId(), upiId.getUpiPaymentType());
        } else {
            System.out.println("You can only select Option 1 or 2 or 3");
        }
